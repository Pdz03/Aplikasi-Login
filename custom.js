...
document.addEventListener('DOMContentLoaded', () => {
    ...
    ///UNTUK ONLINE
    // var pwaScope = "/aplikasi-kos/";
    // var pwaLocation = "https://demo.sevenkos.com/_service-worker.js";
    ...
    function init_template(){
        ...
        if (backButton.length) {
            backButton.forEach(el => el.addEventListener('click', e => {
                ...
                // Cek panjang riwayat browser
                if (window.history.length > 1) {
                    // Kembali ke halaman sebelumnya
                    window.history.back();
                } else {
                    // Jika tidak ada riwayat, arahkan ke halaman default
                    window.location.href = '/hq/Home'; // Ganti dengan URL home Anda
                }
            }));
        }
        //Back to Top
        ...
        window.addEventListener('scroll', function() {
            if (document.querySelectorAll('.scroll-ad, .header-auto-show, .header-bar, #footer-bar').length) {
				...
				if(passed_header){
					if(document.querySelectorAll('.header-bar').length > 0 && document.querySelectorAll('#footer-bar').length > 0){
